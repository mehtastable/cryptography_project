# C++ and Python Random Generator Test Using NIST Test Suite

## Description
This project was created for a cryptography class.

It tests 2 C++ and 2 Python random number generators (RNGs) against the NIST test suite for random numbers.
    1) C++ std::mt19937
    2) C++ std::random_device
    3) Python random.getrandbits
    4) Python os.urandom
    
The first three NIST tests were recreated and used to test the generated sequences.
    1) Frequency (Monobit) Test
    2) Frequency Within a Block Test
    3) Runs Test

## Installation
If using Codespaces:
    Python virtual environment should download the required packages using the requirements.txt.

If running locally:
    1) Navigate to the project directory
    2) Activate virtual environment
        `source venv/bin/activate`
    3) Install the requirements
        `pip install -r requirements.txt
    4) Deactivate the virtual environment

## Running the program
Using the terminal/command line
    1) Run the bash script run_experiment.sh
        `./run_experiment.sh`
    2) Enter the number of trials to run. (NIST recommends 100 or more trials)
    3) Enter the number of bits to generate. (NIST recommends 1,000,000 or more bits)

The program will run the requested number of trials, generating the requested number of bits each time and saved in the data folder.

The bit sequences will be tested using the three NIST tests above.

The results will be save as .txt, .csv, and .png files in the results folder.

## Future Work
1) Recreate all 15 tests into the pipeline.
2) Implement the NIST sts tool in the pipeline to ensure that my implementation of the tests are accurate.
